...
docReady(async function () {
  ...
  gel("manual_add").addEventListener(
    ...
    (e) => {
      ...
      gel("wpa_ssid-pwd").textContent = selectedSSID;
      gel("eap_ssid-pwd").textContent = selectedSSID;
      wifi_div.style.display = "none";
  ...
  gel("wifi-list").addEventListener(
    ...
    (e) => {
      ...
      /* —— Go to the last child of the clicked element —— */
      let security;
      el = e.target;
      ...
      for (let i = 0; i < el.classList.length; i++) {
        if (el.classList[i].startsWith("auth")) {
          security = el.classList[i].substring(4)
          break;
        }
      }
  ...
  gel("cancel_open").addEventListener("click", cancel, false);
  gel("cancel_wpa").addEventListener("click", cancel, false);
  gel("cancel_eap").addEventListener("click", cancel, false);
  gel("manual_cancel_wpa").addEventListener("click", cancel, false);
  gel("manual_cancel_eap").addEventListener("click", cancel, false);
  ...
  gel("join_open").addEventListener(
    "click",
    (e) => {
      performConnect("wpa");
    },
...
async function performConnect(conntype) {
  ...
  await fetch("connect.json", {
    ...
    headers: {
      "Content-Type": "application/json",
      "X-Custom-ssid": encodeURIComponent(selectedSSID),
      "X-Custom-username": encodeURIComponent(username),
      "X-Custom-pwd": encodeURIComponent(pwd),
    },
    body: { timestamp: Date.now() },
  });
...
function refreshAPHTML(data) {
  ...
  data.forEach(function (e, idx, array) {
    ...
    h += `<div class="ape${ap_class}"><div class="${rssicon}"><div class="auth${e.auth} ${auth}">${e.ssid}</div></div></div>\n`;
  });
